...
<x-layouts::auth.card>
    <div class="flex flex-col gap-8">
        <x-auth-header :title="__('Shipper registration')" :description="__('Create your company account to manage shipments and bookings.')" />
        ...
        <form method="POST" action="{{ route('register.store') }}" class="flex flex-col gap-8">
            ...
            <section class="space-y-4">
                ...
                <div class="space-y-4">
                    <flux:input name="name" :label="__('Your name')" type="text" required autofocus autocomplete="name"
                        :value="old('name')" icon:leading="user"
                        class="[&_[data-flux-icon]]:text-sky-600 dark:[&_[data-flux-icon]]:text-cyan-300" />

                    <flux:input name="email" :label="__('Email address')" type="email" required autocomplete="username"
                        :value="old('email')" icon:leading="envelope"
                        class="[&_[data-flux-icon]]:text-sky-600 dark:[&_[data-flux-icon]]:text-cyan-300" />
                </div>
            </section>
            ...
            <section class="space-y-4">
                ...
                <div class="space-y-4">
                    <flux:input name="company_name" :label="__('Company name (optional)')" type="text"
                        :value="old('company_name')" icon:leading="building-office-2"
                        class="[&_[data-flux-icon]]:text-sky-600 dark:[&_[data-flux-icon]]:text-cyan-300" />

                    <x-phone-input name="phone" :label="__('Phone')" required :value="old('phone')" />

                    <flux:input name="address" :label="__('Address')" type="text" required :value="old('address')"
                        autocomplete="street-address" icon:leading="map-pin"
                        class="[&_[data-flux-icon]]:text-sky-600 dark:[&_[data-flux-icon]]:text-cyan-300" />
                </div>
            </section>
            ...
            <section class="space-y-4">
                ...
                <livewire:auth.register-geo-selects :initial-country-id="old('country_id')"
                    :initial-state-id="old('state_id')" :initial-city-id="old('city_id')" />
            </section>
            ...
            <section class="space-y-4">
                ...
                <div class="space-y-4">
                    <flux:input name="password" :label="__('Password')" type="password" required
                        autocomplete="new-password" viewable icon:leading="lock-closed"
                        class="[&_[data-flux-icon]]:text-sky-600 dark:[&_[data-flux-icon]]:text-cyan-300" />

                    <flux:input name="password_confirmation" :label="__('Confirm password')" type="password" required
                        autocomplete="new-password" viewable icon:leading="key"
                        class="[&_[data-flux-icon]]:text-sky-600 dark:[&_[data-flux-icon]]:text-cyan-300" />
                </div>
            </section>
            ...
            <flux:field variant="inline">
                ...
                <flux:label class="inline-flex flex-wrap items-center gap-1">
                    <span class="text-sm">{{ __('I agree to the') }}</span>

                    <flux:link :href="route('terms')" class="text-sm">
                        {{ __('terms of service') }}
                    </flux:link>

                    <span class="text-sm">{{ __('and') }}</span>

                    <flux:link :href="route('privacy')" class="text-sm">
                        {{ __('privacy policy') }}
                    </flux:link>
