...
<div>
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div class="flex items-center gap-3">
            <flux:button wire:click="$set('showTopUpModal', true)" variant="primary" icon="plus">Fund Wallet
            </flux:button>
        </div>
    </div>
    ...
    <!-- Balance Card -->
    <div
        class="mb-8 relative overflow-hidden bg-zinc-900 dark:bg-black rounded-3xl p-8 md:p-10 shadow-xl border border-zinc-800 group hover:shadow-2xl transition-all">
        <!-- Abstract gradient blur effects -->
        <div
            class="absolute -right-20 -top-20 w-64 h-64 rounded-full bg-emerald-500/20 blur-3xl group-hover:bg-emerald-500/30 transition-colors duration-500">
        </div>
        <div
            class="absolute -left-10 -bottom-10 w-48 h-48 rounded-full bg-indigo-500/20 blur-2xl group-hover:bg-indigo-500/30 transition-colors duration-500">
        </div>
        ...
        <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
            ...
            <div
                class="shrink-0 flex items-center justify-center p-5 bg-white/5 backdrop-blur-md rounded-2xl border border-white/10 hover:bg-white/10 transition-colors">
        ...
        <div>
            ...
            <flux:table :paginate="$this->topUps">
                ...
                <flux:table.rows>
                    ...
                    @empty
                        <flux:table.row>
                            <flux:table.cell colspan="3" class="text-center text-zinc-500">No funding requests found.
                            </flux:table.cell>
                        </flux:table.row>
                    @endforelse
                </flux:table.rows>
                ...
                <flux:table.rows>
                    @forelse($this->transactions as $tx)
                        <flux:table.row :key="$tx->id">
                            <flux:table.cell>{{ Str::limit($tx->description ?? $tx->reference ?? '-', 20) }}
                            </flux:table.cell>
                            <flux:table.cell>
                            ...
                            <flux:table.cell
                                class="font-medium {{ $tx->type->value === 'credit' ? 'text-emerald-600' : 'text-zinc-900' }}">
                                {{ $tx->type->value === 'credit' ? '+' : '-' }}${{ number_format($tx->amount, 2) }}
                            </flux:table.cell>
                        </flux:table.row>
                    @empty
                        <flux:table.row>
                            <flux:table.cell colspan="3" class="text-center text-zinc-500">No transactions recorded.
                            </flux:table.cell>
                        </flux:table.row>
                    @endforelse
                </flux:table.rows>
    ...
    <flux:modal wire:model="showTopUpModal" class="md:w-[32rem]">
        <form wire:submit="requestTopUp" class="space-y-6">
            ...
            <flux:input wire:model="amount" label="Amount (USD)" type="number" step="0.01" required
                icon="currency-dollar" placeholder="1000.00" />
            <flux:input wire:model="reference" label="Bank Reference / Memo (Optional)" placeholder="e.g. TR-98765" />

            <flux:input type="file" wire:model="receipt" label="Transfer Receipt" required
