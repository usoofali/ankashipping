...
<div>
    <x-crud.page-shell>
        <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-4">
            <x-crud.page-header :heading="__('Wallet Funding Approvals')" :subheading="__('Review and verify top-up requests.')" icon="banknotes" class="!mb-0" />
                ...
                <flux:table.rows>
                    @forelse($this->topUps as $topUp)
                        <flux:table.row :key="$topUp->id">
                            <flux:table.cell class="whitespace-nowrap">{{ $topUp->created_at->format('M d, Y H:i') }}
                            </flux:table.cell>
                            <flux:table.cell>
                            ...
                            <flux:table.cell>
                                <a href="{{ Storage::url($topUp->receipt_path) }}" target="_blank"
                                    ...
                                    <flux:dropdown align="end" variant="ghost">
                                        ...
                                        <flux:menu>
                                            @can('wallet_top_ups.approve')
                                                <flux:menu.item wire:click="confirmApprove({{ $topUp->id }})" icon="check"
                                                    class="text-emerald-600 hover:text-emerald-700">Approve</flux:menu.item>
                                            @endcan
                                            @can('wallet_top_ups.reject')
                                                <flux:menu.separator />
                                                <flux:menu.item wire:click="confirmReject({{ $topUp->id }})" icon="x-mark"
                                                    variant="danger">Reject</flux:menu.item>
                                            @endcan
                                        </flux:menu>
                                    </flux:dropdown>
                                @elseif($topUp->status->value === 'rejected')
                                    <div class="text-right">
                                        <span class="block text-xs text-zinc-500">By {{ $topUp->approver?->name ?? 'Admin' }}
                                            ({{ $topUp->approver?->roles?->first()?->name ?? 'Staff' }})</span>
                                        <span class="block text-xs text-red-500 mt-0.5"
                                            title="{{ $topUp->rejection_reason }}">Hover for reason</span>
                                    </div>
                                @else
                                    <span class="text-xs text-zinc-500">By {{ $topUp->approver?->name ?? 'Admin' }}
                                        ({{ $topUp->approver?->roles?->first()?->name ?? 'Staff' }})</span>
                                @endif
                            </flux:table.cell>
                        </flux:table.row>
    ...
    <flux:modal wire:model="showApproveModal" class="max-w-md">
        <form wire:submit="approve" class="space-y-6">
            <div>
                ...
                <flux:subheading>
                    Are you sure you want to officially approve this top-up? The shipper's wallet will be securely
                    credited in the ledger. This action cannot be easily reversed.
                </flux:subheading>
            </div>
    ...
    <flux:modal wire:model="showRejectModal" class="md:w-[32rem]">
        <form wire:submit="reject" class="space-y-6">
            ...
            <flux:textarea wire:model="rejectionReason" label="Rejection Reason" required rows="3"
                placeholder="e.g. Receipt image is blurry, or funds not verified in bank." />
